...
add_slide_button = tk.Button(control_frame, text="Add New Slide", command=add_slide)
add_slide_button.grid(row=3, column=0, padx=5, pady=5)

# Dropdown to select slide
slide_selector = ttk.Combobox(control_frame, values=list(range(len(slides))), state="readonly")
slide_selector.grid(row=3, column=1, padx=5, pady=5)
slide_selector.bind("<<ComboboxSelected>>", lambda e: switch_slide(int(slide_selector.get())))

# Add a canvas for the slide preview
preview_canvas = tk.Canvas(root, width=540, height=540, bg='white')  # Adjusted size for better fit
